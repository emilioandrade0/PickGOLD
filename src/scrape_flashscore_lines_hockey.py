...
RAW_FILE = BASE_DIR / "data" / "odds_provider" / "flashscore_hockey_raw.csv"
...
def _norm_team(value: str) -> str:
    ...
    txt = re.sub(r"\b(fc|cf|club|deportivo|athletic|atletico|the|de|hc)\b", " ", txt)
...
def _build_espn_lookup(days_ahead: int = 3):
...
def _resolve_sport_from_league(league_text: str):
    ...
    return "nhl" if "nhl" in league else None
...
def _collect_flashscore_rows():
    ...
    with sync_playwright() as p:
        ...
        page.goto("https://www.flashscore.com.mx/hockey/", wait_until="networkidle", timeout=120000)
        ...
        for idx in range(row_count):
            ...
            sport = _resolve_sport_from_league(league)
            if sport != "nhl":
                continue
...
def _merge_rows(overrides: pd.DataFrame, rows: list[dict]):
    ...
    if "odds_source_provider" not in inc.columns:
        inc["odds_source_provider"] = "flashscore:hockey"
    ...
    if src_provider in merged.columns:
        ...
        merged.loc[fill_provider, "odds_source_provider"] = merged.loc[fill_provider, src_provider].fillna("flashscore:hockey")
...
def scrape_and_apply(days_ahead: int = 3):
        ...
        out_rows.append(
            {
                ...
                "odds_source_provider": "flashscore:hockey_ml",
            }
        )

    raw = pd.DataFrame(flash_rows)
    if raw.empty:
        raw = pd.DataFrame(columns=["sport", "league", "home_team", "away_team", "home_decimal_odds", "away_decimal_odds", "source_url"])
    ...
    print(f"[OK] Flashscore hockey rows(raw): {len(flash_rows)} -> {RAW_FILE}")
    print(f"[OK] Flashscore hockey rows(mapped): {len(out_rows)}")
