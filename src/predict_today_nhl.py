...
def get_team_snapshot(history_df: pd.DataFrame, team: str, cutoff_date: str) -> Optional[Dict]:
    ...
    # Find last game where team was home or away
...
def build_pregame_features(history_df: pd.DataFrame, upcoming_game: Dict) -> Optional[Dict]:
    ...
    # Build feature dict with home_/away_ prefixes
    features = {}
...
def predict_market(assets: Dict, features_dict: Dict, market: str = None) -> Tuple:
    ...
    # Keep feature names so LightGBM receives the same schema used at fit time.
    X = pd.DataFrame([{f: features_dict.get(f, 0.0) for f in feature_names}], columns=feature_names)
    ...
    # DEBUG
    #print(f"DEBUG: ensemble_probs[:, 1]={ensemble_probs[:, 1]}, threshold={threshold}, preds={preds}, preds[0]={preds[0]}")
    
    # For full_game, remap back: 0 -> 1 (Home Win), 1 -> 2 (Away Win)
    if market == "full_game":
        preds = preds + 1
...
def derive_nhl_first_period_pick(prob_over_55: float) -> Dict:
    ...
    # Re-center around the 5.5 market midpoint to avoid systematic 1P over bias.
...
def predict_today_nhl():
    ...
    # Load history for feature building
    try:
    ...
    if not markets_available:
        print(f"   [ERROR] No models available for prediction")
        return
    ...
    for date_str, group_df in upcoming_df.groupby("date"):
        ...
        for _, row in group_df.iterrows():
            ...
            features = build_pregame_features(history_df, {"date": date_str, "home_team": home_team, "away_team": away_team})
            ...
            if "full_game" in markets_available:
                try:
                    pred, conf, probs = predict_market(markets_available["full_game"], features, market="full_game")
                    full_labels = ["N/A", "Home Win", "Away Win"]
                    game_dict["full_game_pick"] = full_labels[int(pred)]
                    model_prob_pick = float(conf)
                    calibrated_prob_pick = calibrate_probability(model_prob_pick, "nhl", "full_game", calibration_cfg)
                    score = fuse_with_pattern_score(recommendation_score(calibrated_prob_pick), pattern_edge)
            ...
            if "spread_2_5" in markets_available:
                try:
                    pred, conf, probs = predict_market(markets_available["spread_2_5"], features, market="spread_2_5")
                    spread_labels = ["Under 5.5", "Over 5.5"]
                    game_dict["spread_pick"] = spread_labels[int(pred)]
                    game_dict["spread_market"] = "Total Goals O/U 5.5"
                    model_prob_pick = float(conf)
                    calibrated_prob_pick = calibrate_probability(model_prob_pick, "nhl", "spread", calibration_cfg)
                    score = fuse_with_pattern_score(recommendation_score(calibrated_prob_pick), pattern_edge)
                    ...
                    # Alias for semantic consistency: this market is totals, not point spread.
                    game_dict["total_pick"] = game_dict["spread_pick"]
                    game_dict["total_confidence"] = game_dict["spread_confidence"]
                    game_dict["total_recommended_pick"] = game_dict["spread_pick"]
                    game_dict["total_recommended_score"] = game_dict["spread_recommended_score"]

                    # Derived 1P O/U 1.5 signal from total-goals probability.
                    ...
                    print(f"   [WARN] spread error for {game_id}: {e}")
                    game_dict["spread_pick"] = "N/A"
                    game_dict["spread_confidence"] = 0
            
            # Home over predictions
            if "home_over_2_5" in markets_available:
                try:
                    pred, conf, probs = predict_market(markets_available["home_over_2_5"], features, market="home_over_2_5")
                    home_labels = ["Home Under 2.5", "Home Over 2.5"]
                    game_dict["home_over_pick"] = home_labels[int(pred)]
                    model_prob_pick = float(conf)
                    calibrated_prob_pick = calibrate_probability(model_prob_pick, "nhl", "home_over_25", calibration_cfg)
                    score = fuse_with_pattern_score(recommendation_score(calibrated_prob_pick), pattern_edge)
            ...
            # Mark as recommended if high average confidence
            avg_conf = sum(confidences_list) / len(confidences_list) if confidences_list else 0
            game_dict["recommended"] = avg_conf >= 55
            score_candidates = [
                float(game_dict.get("full_game_recommended_score", 0.0)),
                float(game_dict.get("spread_recommended_score", 0.0)),
                float(game_dict.get("home_over_recommended_score", 0.0)),
            ]
