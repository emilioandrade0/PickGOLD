...
def generate_historical_predictions():
    ...
    for test_idx in range(start_idx, len(df)):
        ...
        X_test = pd.DataFrame(
            [{col: test_row.get(col, 0) for col in feature_cols}],
        ...
        game_dict = {
            ...
            # NHL: no usar spread real en este pipeline
            "spread_pick": None,
            "spread_market": None,
            "spread_line": None,
        ...
        # TOTAL 5.5 market (antes estaba mal metido en spread_*)
        if valid_spread.sum() >= 10:
            try:
                ...
                total_pick = total_labels[int(pred[0])]

                game_dict["total_pick"] = total_pick
                game_dict["total_market"] = "Total Goals O/U 5.5"
                game_dict["total_line"] = 5.5
                game_dict["total_confidence"] = int(conf[0] * 100)
                game_dict["total_recommended_pick"] = total_pick
                game_dict["total_recommended_score"] = round(float(conf[0]) * 100, 1)

                over_55_prob = float(prob[0]) if not np.isscalar(prob) else float(prob)
                ...
                if not pd.isna(test_row["TARGET_over_55"]):
                    ...
                    market_stats["spread_2_5"]["total_conf"] += float(conf[0])

            except Exception:
                game_dict["total_pick"] = "ERROR"
                game_dict["total_market"] = "Total Goals O/U 5.5"
                game_dict["total_line"] = 5.5
                game_dict["total_confidence"] = 0
                game_dict["total_recommended_pick"] = "ERROR"
                game_dict["total_recommended_score"] = 0.0
                game_dict["correct_total"] = None
        ...
        if valid_home.sum() >= 10:
            try:
                ...
                game_dict["home_over_recommended_score"] = round(float(conf[0]) * 100, 1)
                ...
                if not pd.isna(test_row["TARGET_home_over_25"]):
                    ...
                    market_stats["home_over_2_5"]["total_conf"] += float(conf[0])
