...
RAW_FILE = BASE_DIR / "data" / "odds_provider" / "flashscore_basketball_raw.csv"
...
def _resolve_sport_from_league(league_text: str):
    ...
    return "nba" if "nba" in league else None
...
def _collect_flashscore_rows():
    ...
    with sync_playwright() as p:
        ...
        page.goto("https://www.flashscore.com.mx/basquetbol/", wait_until="networkidle", timeout=120000)
...
def _merge_rows(overrides: pd.DataFrame, rows: list[dict]):
    ...
    if "odds_source_provider" not in inc.columns:
        inc["odds_source_provider"] = "flashscore:basketball"
    ...
    if src_provider in merged.columns:
        ...
        merged.loc[fill_provider, "odds_source_provider"] = merged.loc[fill_provider, src_provider].fillna("flashscore:basketball")
...
def scrape_and_apply(days_ahead: int = 3):
        ...
        out_rows.append(
            {
                ...
                "odds_source_provider": "flashscore:basketball_ml",
            }
        )
    ...
    print(f"[OK] Flashscore basketball rows(raw): {len(flash_rows)} -> {RAW_FILE}")
    print(f"[OK] Flashscore basketball rows(mapped): {len(out_rows)}")
