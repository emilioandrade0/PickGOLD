...
from pattern_engine_bundesliga import generate_bundesliga_patterns
...
UPCOMING_FILE = BASE_DIR / "data" / "bundesliga" / "raw" / "bundesliga_upcoming_schedule.csv"
...
def build_output_rows(df_day: pd.DataFrame, schedule_df: pd.DataFrame, raw_history_df: pd.DataFrame):
    ...
    for i, row in df_day.reset_index(drop=True).iterrows():
        ...
        liga_patterns = generate_bundesliga_patterns(row.to_dict())
...
def main():
    ...
    print(f"\n[OK] Total predicciones Bundesliga: {total_predictions}")
