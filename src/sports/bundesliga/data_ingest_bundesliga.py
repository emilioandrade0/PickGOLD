...
BASE_DIR = SRC_ROOT
RAW_DATA_DIR = BASE_DIR / "data" / "bundesliga" / "raw"
RAW_DATA_DIR.mkdir(parents=True, exist_ok=True)

FILE_PATH_ADVANCED = RAW_DATA_DIR / "bundesliga_advanced_history.csv"
FILE_PATH_UPCOMING = RAW_DATA_DIR / "bundesliga_upcoming_schedule.csv"
...
EXTERNAL_ODDS_FILE = Path(r"C:\Users\andra\Desktop\Historical\data\exports\historical-odds-alemania-bundesliga.csv")
...
ESPN_SUMMARY_URL = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/summary?event={event_id}"
)
...
def load_external_historical_odds() -> pd.DataFrame:
    ...
    try:
        ...
        print(f"[WARN] No se pudo leer CSV de odds Bundesliga: {exc}")
        return pd.DataFrame()
...
def fetch_upcoming_schedule_for_range(start_date: str, days_ahead: int = UPCOMING_DAYS_AHEAD):
    ...
    for day_offset in range(days_ahead + 1):
        ...
        try:
            ...
            print(f"   ðŸ“… Agenda Bundesliga {day_str}: {day_count} juegos")

        except Exception as e:
            print(f"âš ï¸ Error descargando agenda Bundesliga {day_str}: {e}")
...
def extract_advanced_espn_data():
    ...
    print("ðŸš€ Iniciando Extractor Avanzado de ESPN Bundesliga...")
    ...
    print("\nðŸ“Š RESUMEN DE ACTUALIZACIÃ“N Bundesliga")
    ...
    if not df_upcoming.empty:
        ...
        print("âš ï¸ No se encontraron juegos en ventana rolling de Bundesliga.")
