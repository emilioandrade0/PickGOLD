...
BASE_DIR = SRC_ROOT
INPUT_FILE = BASE_DIR / "data" / "bundesliga" / "processed" / "model_ready_features_bundesliga.csv"
...
def load_dataset() -> pd.DataFrame:
    if not INPUT_FILE.exists():
        raise FileNotFoundError(f"No existe el archivo de features Bundesliga: {INPUT_FILE}")
...
def train_single_market(df: pd.DataFrame, market_key: str, target_col: str, feature_cols: List[str], problem_type: str) -> Dict:
    ...
    print(f"\nðŸš€ Entrenando mercado Bundesliga: {market_key}")
...
def train_all_models() -> Dict:
    """Entrena todos los modelos para Bundesliga."""
    print("=" * 70)
    print("ðŸŽ¯ ENTRENAMIENTO DE MODELOS PARA Bundesliga")
    print("=" * 70)

    df = load_dataset()
    feature_cols = get_feature_columns(df)

    print("\nðŸ“¦ Dataset Bundesliga cargado")
    ...
    summary_path = REPORTS_DIR / "training_summary_bundesliga.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 70)
    print("ðŸ“Š RESUMEN FINAL Bundesliga")
    print("=" * 70)
