...
BASE_DIR = SRC_ROOT
RAW_DATA_DIR = BASE_DIR / "data" / "ligue1" / "raw"
RAW_DATA_DIR.mkdir(parents=True, exist_ok=True)

FILE_PATH_ADVANCED = RAW_DATA_DIR / "ligue1_advanced_history.csv"
FILE_PATH_UPCOMING = RAW_DATA_DIR / "ligue1_upcoming_schedule.csv"
...
EXTERNAL_ODDS_FILES = [
    Path(r"C:\Users\andra\Desktop\Historical\data\exports\historical-odds-francia-ligue-1.csv"),
    Path(r"C:\Users\andra\Desktop\Historical\data\exports\historical-odds-francia-ligue1.csv"),
]
...
ESPN_SUMMARY_URL = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/fra.1/summary?event={event_id}"
)
...
def load_external_historical_odds() -> pd.DataFrame:
    odds_file = next((p for p in EXTERNAL_ODDS_FILES if p.exists()), None)
    ...
    try:
        ...
        print(f"[WARN] No se pudo leer CSV de odds ligue1: {exc}")
        return pd.DataFrame()
...
def enrich_with_external_odds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_df = df.copy()
    merged = base_df.merge(odds_df, on=["date", "home_team", "away_team"], how="left", suffixes=("", "_csv"))
...
def fetch_upcoming_schedule_for_range(start_date: str, days_ahead: int = UPCOMING_DAYS_AHEAD):
    ...
    for day_offset in range(days_ahead + 1):
        ...
        try:
            ...
            print(f"   ðŸ“… Agenda ligue1 {day_str}: {day_count} juegos")

        except Exception as e:
            print(f"âš ï¸ Error descargando agenda ligue1 {day_str}: {e}")
...
def extract_advanced_espn_data():
    ...
    print("ðŸš€ Iniciando Extractor Avanzado de ESPN ligue1...")
    ...
    print("\nðŸ“Š RESUMEN DE ACTUALIZACIÃ“N ligue1")
    ...
    if not df_upcoming.empty:
        ...
        print("âš ï¸ No se encontraron juegos en ventana rolling de ligue1.")
