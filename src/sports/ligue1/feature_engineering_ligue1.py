...
BASE_DIR = SRC_ROOT
RAW_DATA = BASE_DIR / "data" / "ligue1" / "raw" / "ligue1_advanced_history.csv"
PROCESSED_DATA_DIR = BASE_DIR / "data" / "ligue1" / "processed"
PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = PROCESSED_DATA_DIR / "model_ready_features_ligue1.csv"
...
def calculate_elo_ratings(df: pd.DataFrame, k: float = 16, home_advantage: float = 50) -> pd.DataFrame:
    ...
    """
    print("ðŸ“ˆ Calculando Sistema ELO para ligue1...")
...
def calculate_team_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print("âš™ï¸ Generando variables generales por equipo (ligue1)...")
...
def calculate_surface_split_features(df: pd.DataFrame):
    ...
    print("ðŸ âœˆï¸ Generando splits Home/Away (ligue1)...")
...
def build_features() -> pd.DataFrame:
    ...
    print("\nðŸ”¨ Construyendo dataset de features para ligue1...")
...
def main():
    print("=" * 70)
    print("ðŸŽ¯ FEATURE ENGINEERING PARA ligue1")
    print("=" * 70)

    if not RAW_DATA.exists():
        raise FileNotFoundError(f"No existe el archivo: {RAW_DATA}\nPrimero corre data_ingest_ligue1.py")
