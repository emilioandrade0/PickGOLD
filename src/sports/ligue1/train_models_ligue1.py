...
BASE_DIR = SRC_ROOT
INPUT_FILE = BASE_DIR / "data" / "ligue1" / "processed" / "model_ready_features_ligue1.csv"

MODELS_DIR = BASE_DIR / "data" / "ligue1" / "models"
MODELS_DIR.mkdir(parents=True, exist_ok=True)

REPORTS_DIR = BASE_DIR / "data" / "ligue1" / "reports"
...
def load_dataset() -> pd.DataFrame:
    if not INPUT_FILE.exists():
        raise FileNotFoundError(f"No existe el archivo de features ligue1: {INPUT_FILE}")
...
def train_single_market(df: pd.DataFrame, market_key: str, target_col: str, feature_cols: List[str], problem_type: str) -> Dict:
    ...
    if market_key != "full_game":
        model_feature_cols = [c for c in model_feature_cols if c not in ODDS_ONLY_FULL_GAME_FEATURES]
    ...
    print(f"\nðŸš€ Entrenando mercado ligue1: {market_key}")
    ...
    if problem_type == "binary":
        if y_train.nunique() < 2:
            ...
            print(f"   Single-class detectado ({constant_class}). Se usa fallback constante.")
        ...
        print(f"   âœ… ROC AUC ens.   : {ensemble_metrics['roc_auc']:.4f}")
        print(
            f"   âœ… Pesos ens.     : XGB={ensemble_weights['xgb_weight']:.2f}, "
...
def train_all_models() -> Dict:
    """Entrena todos los modelos para ligue1."""
    print("=" * 70)
    print("ðŸŽ¯ ENTRENAMIENTO DE MODELOS PARA ligue1")
    print("=" * 70)

    df = load_dataset()
    feature_cols = get_feature_columns(df)

    print("\nðŸ“¦ Dataset ligue1 cargado")
    ...
    summary_path = REPORTS_DIR / "training_summary_ligue1.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 70)
    print("ðŸ“Š RESUMEN FINAL ligue1")
    print("=" * 70)
