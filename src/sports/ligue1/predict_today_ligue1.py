...
from pattern_engine_ligue1 import generate_ligue1_patterns
from pick_selector import fuse_with_pattern_score, recommendation_score
from event_adjustments_ligue1 import (
...
FEATURES_FILE = BASE_DIR / "data" / "ligue1" / "processed" / "model_ready_features_ligue1.csv"
UPCOMING_FILE = BASE_DIR / "data" / "ligue1" / "raw" / "ligue1_upcoming_schedule.csv"
RAW_HISTORY_FILE = BASE_DIR / "data" / "ligue1" / "raw" / "ligue1_advanced_history.csv"
MODELS_DIR = BASE_DIR / "data" / "ligue1" / "models"
...
def load_market_assets(market_key: str):
    """Carga modelos, features y metadata de un mercado especifico."""
...
def predict_market(df: pd.DataFrame, market_key: str):
    """Predice probabilidades usando ensemble con modelos disponibles."""
    xgb, lgbm, lgbm_secondary, catboost, feature_columns, metadata, threshold = load_market_assets(market_key)
    ...
    model_count = 3 + (1 if catboost is not None else 0)

    if market_key == "full_game":
        # Para multiclass: promedio de modelos disponibles
        ...
        # Para binary (over_25, btts): promedio de modelos disponibles
...
def build_output_rows(df_day: pd.DataFrame, schedule_df: pd.DataFrame, raw_history_df: pd.DataFrame):
    """Construye las predicciones finales usando los modelos ML."""
    fg_probs, fg_preds, _, _ = predict_market(df_day, "full_game")
    over_probs, over_preds, over_threshold, _ = predict_market(df_day, "over_25")
    btts_probs, btts_preds, btts_threshold, _ = predict_market(df_day, "btts")
    try:
        corners_probs, corners_preds, corners_threshold, _ = predict_market(df_day, "corners_over_95")
    ...
    for i, row in df_day.reset_index(drop=True).iterrows():
        ...
        liga_patterns = generate_ligue1_patterns(row.to_dict())
        ...
        output.append(
            {
                ...
                "status_state": "" if sched_row is None or pd.isna(sched_row.get("status_state")) else str(sched_row.get("status_state") or ""),
                "status_description": "" if sched_row is None or pd.isna(sched_row.get("status_description")) else str(sched_row.get("status_description") or ""),
            }
        )
...
def main():
    ...
    for date_str, sched in schedule_df.groupby("date"):
        ...
        predictions = build_output_rows(df_features, sched, raw_history_df)
    ...
    print(f"\n[OK] Total predicciones ligue1: {total_predictions}")
