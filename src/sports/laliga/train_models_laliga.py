...
BASE_DIR = SRC_ROOT
INPUT_FILE = BASE_DIR / "data" / "laliga" / "processed" / "model_ready_features_laliga.csv"

MODELS_DIR = BASE_DIR / "data" / "laliga" / "models"
MODELS_DIR.mkdir(parents=True, exist_ok=True)

REPORTS_DIR = BASE_DIR / "data" / "laliga" / "reports"
...
def load_dataset() -> pd.DataFrame:
    if not INPUT_FILE.exists():
        raise FileNotFoundError(f"No existe el archivo de features LaLiga EA Sports: {INPUT_FILE}")
...
def train_single_market(df: pd.DataFrame, market_key: str, target_col: str, feature_cols: List[str], problem_type: str) -> Dict:
    ...
    print(f"\nðŸš€ Entrenando mercado LaLiga EA Sports: {market_key}")
...
def train_all_models() -> Dict:
    """Entrena todos los modelos para LaLiga EA Sports."""
    print("=" * 70)
    print("ðŸŽ¯ ENTRENAMIENTO DE MODELOS PARA LaLiga EA Sports")
    print("=" * 70)

    df = load_dataset()
    feature_cols = get_feature_columns(df)

    print("\nðŸ“¦ Dataset LaLiga EA Sports cargado")
    ...
    summary_path = REPORTS_DIR / "training_summary_laliga.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 70)
    print("ðŸ“Š RESUMEN FINAL LaLiga EA Sports")
    print("=" * 70)
