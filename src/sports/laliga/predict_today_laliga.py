...
from pattern_engine_laliga import generate_laliga_patterns
from pick_selector import fuse_with_pattern_score, recommendation_score
from event_adjustments_laliga import (
...
FEATURES_FILE = BASE_DIR / "data" / "laliga" / "processed" / "model_ready_features_laliga.csv"
UPCOMING_FILE = BASE_DIR / "data" / "laliga" / "raw" / "laliga_upcoming_schedule.csv"
RAW_HISTORY_FILE = BASE_DIR / "data" / "laliga" / "raw" / "laliga_advanced_history.csv"
MODELS_DIR = BASE_DIR / "data" / "laliga" / "models"
...
def build_output_rows(df_day: pd.DataFrame, schedule_df: pd.DataFrame, raw_history_df: pd.DataFrame):
    ...
    for i, row in df_day.reset_index(drop=True).iterrows():
        ...
        liga_patterns = generate_laliga_patterns(row.to_dict())
...
def main():
    ...
    print(f"\n[OK] Total predicciones LaLiga EA Sports: {total_predictions}")
