...
from sports.wnba.pattern_engine_wnba import generate_wnba_patterns
...
def fetch_games_for_date(target_dt):
    ...
    url = f"https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/scoreboard?dates={target_date_str}"
    ...
    for event in events:
        ...
        h_abbr = ESPN_TO_WNBA.get(home_data["team"]["abbreviation"], home_data["team"]["abbreviation"])
        a_abbr = ESPN_TO_WNBA.get(away_data["team"]["abbreviation"], away_data["team"]["abbreviation"])
...
def fetch_upcoming_games(days_ahead: int = 14):
    ...
    for day_offset in range(days_ahead + 1):
        ...
        try:
            ...
            print(f"📅 {LEAGUE_LABEL} {day_dt}: {len(df_day)} juegos")
        except Exception as e:
            print(f"⚠️ Error {LEAGUE_LABEL} {day_dt}: {e}")
...
def predict_today():
    print(f"🔮 INICIANDO MOTOR IA {LEAGUE_LABEL}...")
    ...
    if upcoming_games.empty:
        print(f"📭 No hay partidos en ventana rolling {LEAGUE_LABEL}.")
        return
    ...
    for date_str, games_df in upcoming_games.groupby("date"):
        ...
        for i, row in df_predict.iterrows():
            ...
            wnba_patterns = generate_wnba_patterns(row.to_dict())
            pattern_edge = aggregate_pattern_edge(wnba_patterns)
            ...
            predictions_output.append(
                {
                    ...
                    "full_game_detected_patterns": wnba_patterns,
    ...
    print(f"\n✅ Total juegos {LEAGUE_LABEL} predichos (rolling): {total_games}")
