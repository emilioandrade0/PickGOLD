...
def predict_for_date(
    ...
    if calibration_cfg is not None:
        calibrated_meta_game = np.array(list(map(lambda v: calibrate_probability(float(v), sport="nba", market="full_game", calibration_config=calibration_cfg), prob_game)))
    else:
    ...
    if calibration_cfg is not None:
        calibrated_meta_q1 = np.array(list(map(lambda v: calibrate_probability(float(v), sport="nba", market="q1", calibration_config=calibration_cfg), prob_q1)))
    else:
    ...
    if calibration_cfg is not None:
        calibrated_meta_h1 = np.array(list(map(lambda v: calibrate_probability(float(v), sport="nba", market="h1", calibration_config=calibration_cfg), prob_h1)))
    else:
    ...
    if bool(models.get("has_spread_models", False)):
        ...
        if calibration_cfg is not None:
            calibrated_meta_spread = np.array(list(map(lambda v: calibrate_probability(float(v), sport="nba", market="spread", calibration_config=calibration_cfg), prob_spread)))
        else:
    ...
    if bool(models.get("has_total_models", False)):
        ...
        if calibration_cfg is not None:
            calibrated_meta_total = np.array(list(map(lambda v: calibrate_probability(float(v), sport="nba", market="total", calibration_config=calibration_cfg), prob_total)))
        else:
...
if __name__ == "__main__":
    ...
    print(f"Generando historial desde {start_date} hasta {end_date}...")
    mode = str(os.getenv("NBA_HIST_MODE", "hybrid")).strip().lower()
    if mode == "precise":
