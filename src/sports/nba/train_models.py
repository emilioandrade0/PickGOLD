...
def search_best_weights(p_x: np.ndarray, p_l: np.ndarray, p_c: np.ndarray, y_true: pd.Series, calib_cfg=None, sport="nba", market="full_game", step: float = 0.1):
...
def train_all_models():
    ...
    def train_for_dataframe(X_df: pd.DataFrame, y_game_ser: pd.Series, y_q1_ser: pd.Series, y_h1_ser: pd.Series, suffix: str = "") -> dict:
        ...
        if calib_cfg is not None:
            calibrated_probs = np.array([calibrate_probability(float(v), sport="nba", market="full_game", calibration_config=calib_cfg) for v in final_probs])
        else:
        ...
        best_w, best_w_acc, best_w_probs = search_best_weights(p_x_test, p_l_test, p_c_test, yg_test, calib_cfg, sport="nba", market="full_game", step=0.1)
        ...
        if calib_cfg is not None:
            calibrated_probs_q = np.array([calibrate_probability(float(v), sport="nba", market="q1", calibration_config=calib_cfg) for v in final_probs_q])
        else:
        ...
        best_w_q, best_w_acc_q, best_w_probs_q = search_best_weights(p_x_q_test, p_l_q_test, p_c_q_test, yq_test, calib_cfg, sport="nba", market="q1", step=0.1)
        ...
        if mask_h1_df.any():
            ...
            if calib_cfg is not None:
                calibrated_probs_h1 = np.array([calibrate_probability(float(v), sport="nba", market="h1", calibration_config=calib_cfg) for v in final_probs_h1])
            else:
            ...
            best_w_h1, best_w_acc_h1, _ = search_best_weights(p_x_h1_test, p_l_h1_test, p_c_h1_test, y_test_h1, calib_cfg, sport="nba", market="h1", step=0.1)
    ...
    def train_single_market(
        ...
        if calib_cfg is not None:
            final_probs = np.array(
                [calibrate_probability(float(v), sport="nba", market=market_key, calibration_config=calib_cfg) for v in final_probs]
            )
        ...
        best_w, best_w_acc, _ = search_best_weights(
            p_x, p_l, p_c, y_test_m, calib_cfg, sport="nba", market=market_key, step=0.1
        )
