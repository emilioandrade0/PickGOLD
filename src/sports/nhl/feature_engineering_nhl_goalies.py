...
def calculate_goalie_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate goalie features incrementally using only past starts.
    Uses current game's goalie identity as a pregame input, and historical goalie form via shift(1).
    """
    ...
    # keep found/confirmed from raw ingest; only coerce rolling goalie metrics generated here
    goalie_numeric_cols = [
...
def create_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["TARGET_full_game"] = df.apply(
        lambda row: 1 if row["home_score"] > row["away_score"] else 0,
        axis=1,
    )
...
def add_matchup_diff_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "home_goalie_goals_allowed_l5" in df.columns and "away_goalie_goals_allowed_l5" in df.columns:
        ...
        df["goalie_experience_diff"] = (
            df["home_goalie_games_started_before"] - df["away_goalie_games_started_before"]
        )
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in [
        ...
        "home_goalie_found", "away_goalie_found", "home_goalie_confirmed", "away_goalie_confirmed",
    ]:
    ...
    numeric_fill_zero = [
        ...
        if c.startswith("home_") or c.startswith("away_") or c.endswith("_diff") or c.startswith("TARGET_")
    ]
    numeric_fill_zero += [
        "h2h_home_win_rate", "home_elo_pre", "away_elo_pre", "elo_diff",
...
def process_nhl_data():
    print("🏒 Feature Engineering NHL (GOALIES + NO LEAKAGE)")
    print("=" * 60)
