...
OUT_CSV = TMP_DIR / "full_game_reliability_compare_2026-04-15.csv"
OUT_MD = TMP_DIR / "full_game_reliability_compare_2026-04-15.md"
...
SCENARIOS = [
    (
        "reliability_off",
        {
            "NBA_MLB_FULL_GAME_RELIABILITY_ENABLED": "0",
            "NBA_MLB_FULL_GAME_RELIABILITY_SHRINK_ALPHA": "0.12",
            "NBA_MLB_FULL_GAME_RELIABILITY_SIDE_SHIFT": "0.010",
            "NBA_MLB_FULL_GAME_RELIABILITY_CONFLICT_SHIFT": "0.006",
        },
    ),
    (
        "reliability_on",
        {
            "NBA_MLB_FULL_GAME_RELIABILITY_ENABLED": "1",
            "NBA_MLB_FULL_GAME_RELIABILITY_SHRINK_ALPHA": "0.12",
            "NBA_MLB_FULL_GAME_RELIABILITY_SIDE_SHIFT": "0.010",
            "NBA_MLB_FULL_GAME_RELIABILITY_CONFLICT_SHIFT": "0.006",
        },
...
def _read_split_metrics() -> dict:
    if not SPLITS_PATH.exists():
        return {
            ...
            "avg_reliability_enabled_rate_test": float("nan"),
    ...
    return {
        ...
        "avg_reliability_enabled_rate_test": _avg("reliability_enabled_rate_test"),
...
def run_scenarios() -> list[dict]:
    ...
    for idx, (name, overrides) in enumerate(SCENARIOS, start=1):
        ...
        log_file = TMP_DIR / f"full_game_reliability_compare_{name}.log"
        ...
        row = {
            "scenario": name,
            "reliability_enabled": _safe_int(overrides.get("NBA_MLB_FULL_GAME_RELIABILITY_ENABLED"), default=0),
            "reliability_shrink_alpha": _safe_float(overrides.get("NBA_MLB_FULL_GAME_RELIABILITY_SHRINK_ALPHA"), default=0.0),
            "reliability_side_shift": _safe_float(overrides.get("NBA_MLB_FULL_GAME_RELIABILITY_SIDE_SHIFT"), default=0.0),
            "reliability_conflict_shift": _safe_float(overrides.get("NBA_MLB_FULL_GAME_RELIABILITY_CONFLICT_SHIFT"), default=0.0),
            "accuracy": float("nan"),
        ...
        if proc.returncode == 0 and SUMMARY_PATH.exists():
            ...
            print(
                ...
                f"rel_split={_format_float(row['reliability_enabled_split_rate'], 4)}"
            )
        else:
...
def apply_decision_flags(rows: list[dict]) -> list[dict]:
    baseline = next((r for r in rows if r.get("scenario") == "reliability_off" and r.get("exit_code") == 0), None)
    if baseline is None:
        for row in rows:
    ...
    for row in rows:
        ...
        if row.get("scenario") == "reliability_off":
...
def write_outputs(rows: list[dict]) -> None:
    fieldnames = [
        ...
        "avg_reliability_enabled_rate_test",
    ...
    lines = []
    lines.append("# Full Game Reliability Compare (2026-04-15)")
    lines.append("")
    lines.append("| scenario | acc | pub_acc | pub_cov | roi/bet | priced | rel_split | rel_shift | decision | reason |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---|---|")

    for row in rows:
        lines.append(
            "| {scenario} | {acc} | {pub_acc} | {pub_cov} | {roi} | {priced} | {rel_split} | {rel_shift} | {decision} | {reason} |".format(
                ...
                priced=_safe_int(row.get("published_priced_picks"), default=0),
                rel_split=_format_float(_safe_float(row.get("reliability_enabled_split_rate"), default=float("nan")), 4),
                rel_shift=_format_float(_safe_float(row.get("avg_reliability_abs_shift_mean_test_summary"), default=float("nan")), 5),
