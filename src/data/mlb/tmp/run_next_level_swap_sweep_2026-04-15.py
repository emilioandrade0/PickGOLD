...
out_csv = root / 'src/data/mlb/tmp/full_game_next_level_swap_sweep_2026-04-15.csv'
...
scenarios = [
    ('baseline_ref', {}),
    ('swap_form_power_drop_obp', {
        'NBA_MLB_FULL_GAME_EXTRA_FEATURES': 'home_form_power,away_form_power',
        'NBA_MLB_FULL_GAME_DROP_FEATURES': 'diff_player_obp_proxy_L10',
    }),
    ('swap_form_power_drop_obp_slg', {
        'NBA_MLB_FULL_GAME_EXTRA_FEATURES': 'home_form_power,away_form_power',
        'NBA_MLB_FULL_GAME_DROP_FEATURES': 'diff_player_obp_proxy_L10,diff_player_slg_proxy_L10',
    }),
    ('swap_home_contact_drop_obp', {
        'NBA_MLB_FULL_GAME_EXTRA_FEATURES': 'home_baserunners_allowed_L10,home_hits_allowed_L10',
        'NBA_MLB_FULL_GAME_DROP_FEATURES': 'diff_player_obp_proxy_L10',
    }),
    ('swap_vsleague_drop_obp_slg', {
        'NBA_MLB_FULL_GAME_EXTRA_FEATURES': 'home_run_diff_L10_vs_league,away_run_diff_L10_vs_league',
        'NBA_MLB_FULL_GAME_DROP_FEATURES': 'diff_player_obp_proxy_L10,diff_player_slg_proxy_L10',
    }),
    ('swap_form_power_drop_surface', {
        'NBA_MLB_FULL_GAME_EXTRA_FEATURES': 'home_form_power,away_form_power',
...
for i, (name, overrides) in enumerate(scenarios, start=1):
    ...
    log_path = log_dir / f'full_game_next_level_swap_{name}.log'
    ...
    rows.append(row)
    print(f"[{i}/{len(scenarios)}] DONE {name} acc={row['accuracy']:.12f} brier={row['brier']:.12f} pub_acc={row['published_accuracy']:.6f} pub_cov={row['published_coverage']:.6f}")

rows_sorted = sorted(rows, key=lambda r: (-1e9 if math.isnan(r['accuracy']) else -r['accuracy'], 1e9 if math.isnan(r['brier']) else r['brier'], -1e9 if math.isnan(r['published_accuracy']) else -r['published_accuracy']))
fields = ['scenario','accuracy','brier','logloss','coverage','published_accuracy','published_coverage','rows','extra_features','drop_features','log_file','exit_code']
...
for r in rows_sorted:
    print(f"{r['scenario']}: acc={r['accuracy']:.12f} brier={r['brier']:.12f} pub_acc={r['published_accuracy']:.6f} pub_cov={r['published_coverage']:.6f}")
